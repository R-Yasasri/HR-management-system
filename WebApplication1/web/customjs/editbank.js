...
//            alert(response);
    ...
    request.open("GET", "LoadBank", true);
    request.send();
}
...
//    alert(resp);
    ...
    table.innerHTML = "<thead><tr role='row'><th>ID</th><th>Bank Name</th><th>Account</th><th>Address</th><th>Employee id</th><th>Employee Name</th><th></th></tr></thead>";
...
function remove(id) {
    ...
    if (option) {
        ...
        request.open("GET", "DeleteBank?id=" + id, true);
        request.send();
    }
}
...
function searchById() {
    ...
    request.open("GET", "SearchBankById?id=" + search, true);
    request.send();
}
...
function search_two() {
    ...
    request.open("GET", "SearchBankByName?name=" + search, true);
    request.send();
}
...
function edit(rowNo) {
    ...
    var account = rowCells[2].childNodes[0].value;
    var address = rowCells[3].childNodes[0].value;
    var empId = rowCells[4].childNodes[0].value;
...
//    alert(id + name + birthday + nic + email + mobile + address);
    ...
    request.open("GET", "EditBank?id=" + id + "&name=" + name + "&account=" + account + "&address=" + address + "&empId=" + empId, true);
    request.send();
}