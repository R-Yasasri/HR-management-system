...
//    alert(resp);
    ...
    table.innerHTML = "<thead><tr role='row'><th>ID</th><th>Title</th><th>Starting Date</th><th>Finishing Date</th><th>Employee Id</th><th>Employee Namee</th><th></th></tr></thead>";
...
function remove(id) {
    ...
    if (option) {
        ...
        request.open("GET", "DeleteJob?id=" + id, true);
        request.send();
    }
}
...
function searchById() {
    ...
    request.open("GET", "SearchJobById?id=" + search, true);
    request.send();
}
...
function search_two() {
    ...
    request.open("GET", "SearchJobByEmployee?empid=" + search, true);
    request.send();
}
...
function edit(rowNo) {
    ...
    var title = rowCells[1].childNodes[0].value;
    var start = rowCells[2].childNodes[0].value;
...
//    alert(id + name + birthday + nic + email + mobile + address);
    ...
    request.open("GET", "EditJob?id=" + id + "&title=" + title + "&start=" + start + "&end=" + end + "&empId=" + empId, true);
    request.send();
}
...
function search_three() {
    ...
    request.open("GET", "SearchJobByTitle?title=" + search, true);
    request.send();
}