function load() {
    ...
    request.open("GET", "LoadPerformanceFeedback", true);
    request.send();
}
...
//    alert(resp);
    ...
    table.innerHTML = "<thead><tr role='row'><th>ID</th><th>Description</th><th>Person Given</th><th>Employee Id</th><th>Employee Name</th><th></th></tr></thead>";
...
<td><input type='text' value='" + item.personGiven + "' /></td>\n\
<td><input type='number' value='" + item.employeeId + "' /></td>\n\
...
function remove(id) {
    ...
    if (option) {
        ...
        request.open("GET", "DeletePerformanceFeedback?id=" + id, true);
        request.send();
    }
}
...
function searchById() {
    ...
    request.open("GET", "SearchPerformanceFeedbackById?id=" + search, true);
    request.send();
}
...
function search_two() {
    ...
    request.open("GET", "SearchPerformanceFeedbackByEmployee?empid=" + search, true);
    request.send();
}
...
function edit(rowNo) {
    ...
    var desc = rowCells[1].childNodes[0].value;
    var person = rowCells[2].childNodes[0].value;
...
//    alert(id + name + birthday + nic + email + mobile + address);
    ...
    request.open("GET", "EditPerformanceFeedback?id=" + id + "&desc=" + desc + "&person=" + person + "&empid=" + empid, true);
    request.send();
}
...
function search_three() {
    ...
    request.open("GET", "SearchPerformanceFeedbackByPersonGiven?person=" + search, true);
    request.send();
}